...
class TestSed < MiniTest::Unit::TestCase
	...
	def test_ascii
		...
		# テスト実行
		system("ruby sed_ascii.rb #{in_file} > #{out_file}")
	...
	def test_ascii2
		...
		# テスト実行
		system("ruby sed_ascii2.rb #{in_file} > #{out_file}")
		#puts File.read(out_file)
	...
	def test_sjis
		...
		# テスト実行
		system("ruby sed_sjis.rb #{in_file} > #{out_file}")
		#puts File.read(out_file)
	...
	def test_utf8
		...
		# テスト実行
		system("ruby sed_utf8.rb #{in_file} > #{out_file}")
		#puts File.read(out_file)
